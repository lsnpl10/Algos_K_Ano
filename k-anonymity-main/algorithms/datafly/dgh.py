            ...
            with open(dgh_path, 'r') as file:
                for line in file:
                    try:
                        ...
                        continue  # Ignore lines that can't be read or are empty

                    # Check if values is not empty
                    if not values:
                        if previous_line:
                            print(f"Ligne avant la vide ou incorrecte: {previous_line.strip()}")
                            print(f"Ligne vide ou incorrecte: {line.strip()}")
                        continue
